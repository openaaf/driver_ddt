...
static struct stpio_pin *pin_22khz = NULL;
static struct stpio_pin *pin_notlnbpwr = NULL;
#endif
...
static int stv090x_start_search(struct stv090x_state *state)
{
	...
	if (state->dev_ver >= 0x20)
	{
		...
		if (state->srate < 2000000)
		{
			 ...
			 * >= 0x20 and here we checks against <= ... interesting ;)
			 */
			if (state->dev_ver <= 0x20)
			{
				/* Cut 2 */
...
#if defined(VITAMIN_HD5000)
			stpio_set_pin(pin_notlnbpwr, 0); /* on */
#endif
			break;
		}
		default:
									...
									enum stv090x_tuner tuner)
{
	...
	state->mclk = 0;
#if defined(VITAMIN_HD5000)
	pin_22khz = stpio_request_pin(6, 1, "FE_22KHZ", STPIO_OUT);
	pin_notlnbpwr = stpio_request_pin(6, 2, "FE_notLNBPWR", STPIO_OUT);
#endif
